...
/**
 * @brief Resets/Clears the memory of the account_t structure.
 *
 ...
 *
 * @param player A Session to the account_t structure whose memory needs to be cleared.
 */
void struct_memset (account_t *Session) {
    memset(Session->pId, 0, sizeof(account_t));
    memset(Session->name, 0, sizeof(account_t));
    memset(Session->surname, 0, sizeof(account_t));
    memset(Session->username, 0, sizeof(account_t));
    ...
    memset(Session->password, 0, sizeof(account_t));
#endif
    memset(Session->scores.date, 0, sizeof(account_t));
    Session->scores.score = 0;
    memset(Session, 0, sizeof(account_t));
}
...
/**
 * @brief Calls the readRandomQuestion function, writes the 10 randomly generated questions to the struct,
...
void displayQuestions(account_t *player, const char *category) {
    ...
    rc = readQuestions(db, questions, category);
    ...
    do {
        ...
        do {
            ...
            switch (toupper(cgetch())) {
                case 'A' :
                     strncpy(questions[qid].selected_choice, questions[qid].choices[0], BUFSIZE - 1);
                     break;
                case 'B' :
                     strncpy(questions[qid].selected_choice, questions[qid].choices[1], BUFSIZE - 1);
                     break;
                case 'C' :
                     strncpy(questions[qid].selected_choice, questions[qid].choices[2], BUFSIZE - 1);
                     break;
                case 'D' :
                     strncpy(questions[qid].selected_choice, questions[qid].choices[3], BUFSIZE - 1);
                     break;
                default:
                    strncpy(questions[qid].selected_choice, "NIL", BUFSIZE - 1);
            }
        } while(strcmp(questions[qid].selected_choice, "NIL") == 0);
        if (strcmp(questions[qid].selected_choice, questions[qid].correct_answer) == 0) {
            score += 100;
        }
        qid += 1;
    } while (qid < 10);
    ...
    if (score > 0) {
        uint32_t bonus_score = score / 10;
        if (elapsed_time > 180) {
            score += bonus_score + 100;
        } else if (elapsed_time <= 120) {
            score += bonus_score + 500;
        } else {
            score += bonus_score + 300;
        }
    }

    SetCursorPosition(x + 2, y += 2);
    print_success("You scored : %04u", score);
    //Update players score to database.
    if (updateScore(player, score)) {
...
void displayAnswrs(account_t *player, Quiz_t *quest) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        while (check < 10) {
            ...
            if (strcmp(quest[qid].selected_choice, quest[qid].correct_answer) == 0) {
                SetCursorPosition(x + 3, y += 1);
                printf("\033[1;37;42mYour Choice    : %s \033[0m", quest[qid].selected_choice);
                SetCursorPosition(x + 2, y += 1);
                printf("Correct Answer : %s", quest[qid].correct_answer);

            }
            else {
                SetCursorPosition(x + 3, y += 1);
                printf("\033[1;37;41mYour Choice    : %s \033[0m", quest[qid].selected_choice);
                SetCursorPosition(x + 2, y += 1);
                printf("Correct Answer : %s", quest[qid].correct_answer);
            }
            qid += 1;
 ...
 */
bool readQuestions(sqlite3 *db, Quiz_t *questions, const char *tableName) {
    char sql[MAXBUFF];
    snprintf(sql, MAXBUFF, "SELECT * FROM %s ORDER BY RANDOM() LIMIT %d;", tableName, MAXQUESTIONS);
    ...
    int idx = 0;
    while (sqlite3_step(stmt) == SQLITE_ROW && idx < MAXQUESTIONS) {
        const unsigned char *question = sqlite3_column_text(stmt, 1);
        if (strlen((const char*)question) >= MAXBUFF) {
            fprintf(stderr, "Question exceeds buffer size.\n");
            sqlite3_finalize(stmt);
            return false;
        }
        strcpy(questions[idx].question, (const char*)question);

        for (int i = 0; i < 4; i++) {
            const unsigned char *choice = sqlite3_column_text(stmt, 2 + i);
            if (strlen((const char*)choice) >= BUFSIZE) {
                fprintf(stderr, "Choice %d exceeds buffer size.\n", i);
                sqlite3_finalize(stmt);
                return false;
            }
            strcpy(questions[idx].choices[i], (const char*)choice);
        }

        const unsigned char *correct_choice = sqlite3_column_text(stmt, 6);
        if (strlen((const char*)correct_choice) >= BUFSIZE) {
            fprintf(stderr, "Correct choice exceeds buffer size.\n");
            sqlite3_finalize(stmt);
            return false;
        }
        strcpy(questions[idx].correct_answer, (const char*)correct_choice);
...
void print_banner(size_t left, const char *text, size_t right) {
    do {
        for (size_t i = 0; i < left; i++)
            printf("%s", (i % 2) ? "\033[1;31;41m  " : "\033[1;37;47m  ");

        if (NULL != text)
            printf("\033[0m%s", text);

        for (size_t i = 0; i < right; i++)
            printf("%s", (i % 2) ? "\033[1;37;47m  " : "\033[1;31;41m  ");
...
char *getFormattedDatetime(void) {
    time_t t;
    time(&t);
    char *formattedDatetime = (char *)calloc(KEYSIZE, sizeof(char));
